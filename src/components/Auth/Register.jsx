  ...
  const [loading, setLoading] = useState(false);
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        setErrorMsg("");
        setLoading(true);
        console.log("preHandleSubmit")
        const { data, error } = await api.createAccount(newData.get('email'), newData.get('password'), newData.get('firstName') + ' ' + newData.get('lastName'))
        if (!error && data) {
          setMsg(
            "Registration Successful. Check your email to confirm your account"
          );
          console.log("datdaback", data)
        }
        navigate('/login');
      } catch (error) {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-100">
        <div className="rounded-lg px-8 py-6 mx-4 mt-4 text-left bg-white shadow-lg md:w-1/3 lg:w-1/3 sm:w-1/3">
          ...
          {!msg == "" && <Alert color="green" message={msg} />}
          <h3 className="text-2xl font-bold text-center">Join us</h3>
          <form action>
            <div className="mt-4">
              <div>
                <label className="block" htmlFor="FirstName">First Name<label>
                  <input type="text" id="firstName" required placeholder="Name" className="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600" />
                </label></label></div>
              <div>
                <label className="block" htmlFor="LastName">Last Name<label>
                  <input type="text" id="lastName" required placeholder="Last Name" className="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600" />
                </label></label></div>
              <div className="mt-4">
                <label className="block" htmlFor="email">Email<label>
                  <input type="email" id="email" required placeholder="Email" className="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600" />
                </label></label></div>
              <div className="mt-4">
                <label className="block">Password<label>
                  <input type="password" id="password" required placeholder="Password" className="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600" />
                </label></label></div>
              <div className="mt-4">
                <label className="block">Confirm Password<label>
                  <input type="password" id="confirmpassword" required placeholder="Confirm same password" className="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600" />
                </label></label></div>
              {/* <span className="text-xs text-red-400">Password must be same!</span> */}
              <div className="flex">
                <button className="w-full px-6 py-2 mt-4 text-white bg-yellow-600 rounded-lg hover:bg-yellow-800"
                  onClick={() => handleSubmit}>
                  Create Account</button>
              </div>
              <div className="mt-6 text-grey-dark">
                Already have an account?
                <a className="ml-2 text-yellow-600 hover:underline" href="/login">
                  Log in
                </a>
              </div>
            </div>
